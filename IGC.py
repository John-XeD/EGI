...
def waktu():
	now = datetime.now()
	hours = now.hour
	if 4 <= hours < 12:timenow = "Hari Ini Harus Lebih Baik Dari Hari Esok"
	elif 12 <= hours < 15:timenow = "Udah Siang Bro"
	elif 15 <= hours < 18:timenow = "Jangan Lupa Sembahyang"
			...
			with ThreadPoolExecutor(max_workers=40) as shinkai:
				for i in user:
					try:
						...
						continue
			print('\n')
			exit(f'[✓] Lancar Jaya Jaya Selamanya, OK- {H}{len(success)}{N} CP- {K}{len(checkpoint)}{N}')
